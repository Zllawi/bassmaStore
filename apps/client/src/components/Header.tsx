...
export default function Header() {
  ...
  const brandLabel = brandName || "„ Ã— «·√›ﬁ"
      ...
      <div className="container mx-auto flex flex-wrap items-center gap-3 px-4 py-4 md:flex-nowrap">
        <div className="flex w-full items-center gap-3">
          <div className="flex items-center gap-3 md:w-auto">
            <button type="button" className="btn-icon md:hidden" onClick={() => setMenuOpen(true)} aria-label="› Õ «·ﬁ«∆„… «·Ã«‰»Ì…">
              <MenuIcon className="h-5 w-5" />
            </button>
            <Link to="/" className="flex items-center gap-3" aria-label="«·⁄Êœ… ··’›Õ… «·—∆Ì”Ì…">
          ...
          <div className="ml-auto flex items-center gap-2 md:hidden">
            ...
            <button type="button" className="btn-icon" onClick={toggleDrawer} aria-label="› Õ ”·… «·„‘ —Ì« ">
            ...
            <button type="button" className="btn-icon" aria-label=" »œÌ· ‰„ÿ «·Ê«ÃÂ…" onClick={toggleTheme}>
              {theme === "dark" ? <SunIcon className="h-5 w-5" /> : <MoonIcon className="h-5 w-5" />}
            </button>
          </div>
        </div>

        <nav className="hidden w-auto gap-6 text-sm font-medium text-white md:flex" aria-label="—Ê«»ÿ «·√ﬁ”«„">
          {categories.map(cat => (
            <Link
              key={cat.value}
