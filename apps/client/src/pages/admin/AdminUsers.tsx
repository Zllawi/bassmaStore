...
export default function AdminUsers() {
  ...
  const onDelete = (u: User) => {
    if (u.role === 'admin') {
      alert('لا يمكن حذف مستخدم بدور مدير (admin).')
      return
    }
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="card p-4">
        {isLoading ? 'جاري تحميل المستخدمين...' : error ? 'حدث خطأ أثناء الجلب' : (
            ...
            <table className="w-full text-sm">
              <thead className="text-white/60">
                <tr>
                  ...
                  <th className="text-right p-2">أُنشئ في</th>
              ...
              <tbody>
                {(data || []).map((u, idx) => (
                  <tr key={u._id} className="border-t border-white/10">
                    ...
                    <td className="p-2">{u.createdAt ? new Date(u.createdAt).toLocaleDateString('ar-SA') : '-'}</td>
                    <td className="p-2">
