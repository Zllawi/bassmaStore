...
export default function AccountSettings() {
  ...
  useEffect(() => { (async()=>{ try { const r = await api.get('/users/me/addresses'); setList(r.data?.data||[]) } catch { /* keep empty on failure */ } })() }, [])
    ...
    <section className="space-y-6">
      ...
      <div className="card p-4">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-white">العناوين</h2>
          {!editing && (
        ...
        {editing && (
          <div className="grid gap-3 sm:grid-cols-2">
            <input className="input" placeholder="تسمية العنوان (اختياري)" value={form.label || ''} onChange={e => setForm(s => ({ ...s, label: e.target.value }))} />
            ...
            {list.map(a => (
              <li key={a.id} className="card p-3">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <div className="text-white font-medium">{a.label || 'بدون تسمية'} {a.isDefault ? <span className="text-xs text-accent">(افتراضي)</span> : null}</div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {!a.isDefault && <button className="btn bg-white/10" disabled={!a.id} onClick={() => a.id && makeDefault(a.id)}>تعيين كافتراضي</button>}
                    <button className="btn bg-white/10" onClick={() => startEdit(a)}>تعديل</button>
                    <button className="btn bg-red-500/80" disabled={!a.id} onClick={() => a.id && del(a.id)}>حذف</button>
                  </div>
                </div>
              </li>
            ))}
            {list.length === 0 && (
              <li className="text-white/70">لا توجد عناوين بعد.</li>
            )}
          </ul>
        )}
      </div>
